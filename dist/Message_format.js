...
module.exports = (rem, messageRaw) => {
    if (messageRaw == undefined)
        return {};
    if (messageRaw?.message == undefined)
        return messageRaw;
    ...
    message.sender = message.isGroupMsg
    ...
    message.isDocument = "documentMessage".includes(message.type);
    message.isMedia = mediaType.includes(message.type);
    message.type == "templateButtonReplyMessage"
        ? (message.selectedButtonId = message.message.templateButtonReplyMessage.selectedId)
        : message.type == "buttonsResponseMessage"
            ? (message.selectedButtonId = message.message.buttonsResponseMessage.selectedButtonId)
            : "";
    if (message.type == "templateButtonReplyMessage")
        message.selectedButtonIndex = message.message.templateButtonReplyMessage.selectedIndex;
    if (message.type == "listResponseMessage")
        message.selectedRowId = message.message.listResponseMessage.singleSelectReply.selectedRowId;
    if (message.isMedia)
        message.getMedia = async () => {
            const stream = await (0, baileys_1.downloadContentFromMessage)(message.message[message.type], message.type.replace("Message", ""));
    ...
    if (message.isMedia)
        message.mimetype = message.message[message.type].mimetype;
    message.isMedia ? message.message[message.type].caption : "";
    if (message.isMedia && (message.type === "audioMessage" || message.type === "videoMessage"))
        message.duration = message.message[message.type].seconds;
    message.quotedMsg = message.message.contextInfo
        ? message.message.contextInfo.quotedMessage
        : message.message[message.type].contextInfo
            ? message.message[message.type].contextInfo.quotedMessage
    ...
    message.mentionedJidList = message.message.contextInfo
    ...
    if (message.quotedMsg) {
        ...
        message.quotedMsg.id = message.message[message.type].contextInfo
            ? message.message[message.type].contextInfo.stanzaId
            : message.message.contextInfo.stanzaId;
        message.quotedMsg.from = message.message[message.type].contextInfo
            ? message.message[message.type].contextInfo.remoteJid || message.from
            : message.message.contextInfo.remoteJid || message.from;
        message.quotedMsg.sender = message.message[message.type].contextInfo
            ? message.message[message.type].contextInfo.participant || rem.user.id.split("@")[0].split(":")[0] + "@s.whatsapp.net"
            : message.message.contextInfo.participant || rem.user.id.split("@")[0].split(":")[0] + "@s.whatsapp.net";
        ...
        message.quotedMsg.body =
            type === "templateButtonReplyMessage"
                ...
                : type === "listMessage"
                    ? message.quotedMsg.description
                    : type === "buttonsMessage"
                        ? message.quotedMsg.contentText
                        : ["listResponseMessage"].includes(type)
                            ? message.quotedMsg.singleSelectReply.selectedRowId
                            : ["buttonsResponseMessage"].includes(type)
                                ? message.quotedMsg.selectedButtonId
                                : message.quotedMsg.caption || message.quotedMsg.conversation || message.quotedMsg.text || "";
        message.quotedMsg.isMedia = mediaType.includes(message.quotedMsg.type);
        message.quotedMsg.isDocument = "documentMessage".includes(message.quotedMsg.type);
        if (message.quotedMsg.isMedia)
            message.quotedMsg.getMedia = async () => {
                ...
                let messageType = mime.split("/")[0].replace("application", "document")
                    ...
                    : mime.split("/")[0];
                const stream = await (0, baileys_1.downloadContentFromMessage)(message.quotedMsg, messageType);
                let buffer = Buffer.from([]);
                for await (const chunk of stream) {
                    buffer = Buffer.concat([buffer, chunk]);
                }
    ...
    message.body =
        message.message.conversation ||
            ...
            message.message[message.type].selectedDisplayText ||
            message.message[message.type].selectedDisplayText ||
            message.message[message.type].description ||
            message.message[message.type].contentText ||
            message.message[message.type].title; //|| message.message[Object.keys(message.message)[0]].text || message.message[Object.keys(message.message)[1]].text
